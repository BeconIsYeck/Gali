...
abstract class Activity {
	public static double GlobalChance = 0.02;

	public virtual double Diameter { get; set; } = 5;                         // GOOD
	public abstract Range LineInteraction { get; }                   // GOOD
	public virtual double Chance { get; } = 1;                       // GOOD
	public virtual int PerYears { get; } = 1_000_000;
	public virtual Color Color { get; } = Colors.LightGrey;

	public virtual bool Continuous { get; } = true;                  // SORTA
	public int Age { get; set; } = 0;                                // BAD
	public double Max = 5;                                           // BAD
	public Coordinate Location { get; set; } = new Coordinate(0, 0); // SORTA
...
class MantlePlume : Activity { // MantlePlume
	...
	public override int Function(Planet planet, int plateIndex, double intensity) {
		...
		//throw new Exception($"MantlePlume error {pointChange} {sign} {intensity}");
...
class SaltDeposit : Activity { // SaltDeposit
	...
	public override int Function(Planet planet, int plateIndex, double intensity) { // Destroy equatorial temperature and remove points
		var factors = planet.GetFactorsWithinRadius(Location, Diameter / 2);
...
class Building : Activity { // Building
	...
	public override int Function(Planet planet, int plateIndex, double intensity) { // GGs
		var factors = planet.GetFactorsWithinRadius(Location, Diameter / 2);
