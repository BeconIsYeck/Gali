	...
	public void Draw(ICanvas canvas, RectF rect) {
		...
		for (var i = 0; i < factors.GetLength(0); i++) {
			for (var j = 0; j < factors.GetLength(1); j++) {
				...
				//var r = MathX.PercentDifference(planet.Ocean.AverageGlobalTemperature, factor.Temperature) / 100;
				//var r = MathX.PercentDifference(planet.Ocean.AverageDepth, factor.Depth) / 100;
				//var r = MathX.PercentDifference(planet.Ocean.SodiumPPM, factor.Salinity) / 100;
				//var r = factor.Populations.Count != 0 ? 1 : 0;
				...
				if (factorSToggles["Populations"]) {
					//foreach ((var p, var _) in factor.Populations) {
				...
				if (factorSToggles["Temperature"]) {
					...
					//if (t >= 0 && t <= 9) {
					//	colors.Add(Color.FromRgba(40, 150, 200, 1));
					//}
					//else if (t >= 10 && t <= 19) {
					//	colors.Add(Color.FromRgba(40, 200, 140, 1));
					//}
					//else if (t >= 20 && t <= 29) {
					//	colors.Add(Color.FromRgba(40, 240, 200, 1));
					//}
					//else if (t >= 30 && t <= 40) {
					//	colors.Add(Color.FromRgba(70, 255, 255, 1));
					//}
					//else {
					//	colors.Add(Color.FromRgba(255, 0, 0, 1));
					//}

					//colors.Add(Color.FromRgba(
					//	0,
					//	0,
					//	MathX.PercentDifference(planet.Ocean.AverageGlobalTemperature, factor.Temperature) / 100,
					//	0.5
					//));
				}
				...
				if (factorSToggles["Salinity"]) {
					colors.Add(Color.FromRgba(
						0,//MathX.PercentDifference(planet.Ocean.AverageSalinity, factor.Depth) / 100,
				...
				if (factorSToggles["Nitrogen"]) {
					colors.Add(Color.FromRgba(
						...
						0,//MathX.PercentDifference(planet.Ocean.AverageCalcium, factor.Depth) / 100,
						0,//MathX.PercentDifference(planet.Ocean.AverageCalcium, factor.Depth) / 100,
						1
					));
				}
				...
				//a = factor.Populations.Count == 0 ? 0 : 1;
				...
				//var b = MathX.PercentDifference(planet.Ocean.AverageGlobalTemperature, factor.Temperature) / 100;
