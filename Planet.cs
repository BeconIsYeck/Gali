...
class Planet {
	...
	public Planet(string name, int seed, ContentPage contentPage, int populations, double eqTemp, double poleTemp, int numOfPlt, (int, int) pltAm, Rect showRect, double scaleW, double scaleH) {
		...
		//	var range = MathX.RandomInt(5, 50);

		//	var pop = new Population("g", "s", this, 100_000, center, range);
		//	pop.Color = MathX.RandomColor(true, true, true, false, 0.5);
		...
		if (!nothing) {
			...
			for (var x = 0; x < Factors.GetLength(0); x++) {
				for (var y = 0; y < Factors.GetLength(1); y++) {
					...
					//factor.Populations.Clear();

					//var updPops = factor.SimulatePopulations(yearsPassed, Factors, x, y);

					//var p = 0;

					//foreach ((var nX, var nY, var pop) in updPops) {
					//	Factors[nX, nY].Populations.Add(pop);
					//	p++;
					//}
					//if (p > 0) 
					//	Math.Sin(p);
					//factor.Populations = updPops;
				}
			}
			...
			//var lastFactor = new Factor[Factors.GetLength(0), Factors.GetLength(1)];
			...
			//lbl2.Text = $"{Greenhouse} / {GreenhouseMin} {GreenhouseMax}";
			...
			//		//if (lerpT == 0) {
			//		//	factor.Temperature = Math.Pow(MathX.ExponentialLerp1(Ocean.PolarTemperature, Ocean.EquatorialTemperature, lerpT, 6), 1.025);
			//		//}

			//		//factor.Salinity = Ocean.SodiumPPM;
			//		//factor.Oxygen = Ocean.SurfaceOxygenPPM;
			//		//factor.Calcium = Ocean.CalciumPPM;
			//		//factor.Nitrogen = Ocean.NitrogenPPM;
...
//	public bool NorthHemisphere { get; set; } = true;
